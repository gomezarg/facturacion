...
BEGIN
	current_stock := stock FROM product where id = NEW.product;
...
CREATE VIEW vw_sale AS
	...
	FROM
		sale s
		INNER JOIN product pr ON s.product = pr.id;


CREATE OR REPLACE FUNCTION sale (
	IN s_product                  integer,
	IN s_quantity                 integer
) RETURNS integer AS 
$$
	INSERT INTO sale(product, quantity)
		VALUES(s_product, s_quantity)
	RETURNING id;
$$ LANGUAGE sql VOLATILE STRICT;
...
$$
	DELETE FROM purchase WHERE id = s_id;
$$ LANGUAGE sql VOLATILE STRICT;


CREATE OR REPLACE FUNCTION sale_search (
	IN s_date_from                timestamp without time zone DEFAULT date_trunc('month', now()),
	IN s_date_to                  timestamp without time zone DEFAULT now(),
	IN s_product_name             text DEFAULT '%'
) RETURNS SETOF vw_sale AS
$$
	SELECT * FROM vw_sale 
		WHERE name ilike '%' || s_product_name || '%' AND date BETWEEN s_date_from AND s_date_to
	ORDER BY date;
$$ LANGUAGE sql STABLE;
...
$$
	SELECT * FROM vw_sale WHERE id = s_id;
$$ LANGUAGE sql STABLE STRICT;